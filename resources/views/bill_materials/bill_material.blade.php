
    <div class='form-group  @if ($errors->has('product_code')) has-error @endif'>
        <label for='product_code' class='col-sm-2 control-label'>product_code<span style='color:red;'> *</span></label>
        <div class='col-sm-10'>
            {{ Form::text('product_code', null, ['class'=>'form-control','maxlength'=>'20']) }}
            @if ($errors->has('product_code')) <p class="help-block">{{ $errors->first('product_code') }}</p> @endif
        </div>
    </div>

    <div class='form-group  @if ($errors->has('bom_product_code')) has-error @endif'>
        <label for='bom_product_code' class='col-sm-2 control-label'>bom_product_code<span style='color:red;'> *</span></label>
        <div class='col-sm-10'>
            {{ Form::text('bom_product_code', null, ['class'=>'form-control','maxlength'=>'20']) }}
            @if ($errors->has('bom_product_code')) <p class="help-block">{{ $errors->first('bom_product_code') }}</p> @endif
        </div>
    </div>

    <div class='form-group  @if ($errors->has('bom_quantity')) has-error @endif'>
        <label for='bom_quantity' class='col-sm-2 control-label'>bom_quantity<span style='color:red;'> *</span></label>
        <div class='col-sm-10'>
            {{ Form::text('bom_quantity', null, ['class'=>'form-control','placeholder'=>'',]) }}
            @if ($errors->has('bom_quantity')) <p class="help-block">{{ $errors->first('bom_quantity') }}</p> @endif
        </div>
    </div>

    <div class='form-group'>
        <div class="col-sm-offset-2 col-sm-10">
            <a class="btn btn-default" href="/bill_materials" role="button">Cancel</a>
            {{ Form::submit('Save', ['class'=>'btn btn-primary']) }}
        </div>
    </div>
