    ...
    <div class='form-group  @if ($errors->has('consult_id')) has-error @endif'>
        <label for='consult_id' class='col-sm-2 control-label'>consult_id<span style='color:red;'> *</span></label>
        <div class='col-sm-10'>
            {{ Form::text('consult_id', null, ['class'=>'form-control','placeholder'=>'',]) }}
            @if ($errors->has('consult_id')) <p class="help-block">{{ $errors->first('consult_id') }}</p> @endif
        </div>
    </div>
